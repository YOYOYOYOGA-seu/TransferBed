...
//---------------------------FRESH by matrix----------------------
//static uint16_t LCD_color[240][320]={0};
//LCD_coordinate LCD_cord[76800]={0};
uint16_t fresh_index = 0;
//LCD的画笔颜色和背景色	   
...
u16 LCD_ReadPoint(u16 x,u16 y)
{
	...
	if(lcddev.id==0X9325||lcddev.id==0X4535||lcddev.id==0X4531||lcddev.id==0X8989||lcddev.id==0X8997||lcddev.id==0XB505)return r;	//这几种IC直接返回颜色值
...
//9320/9325/9328/4531/4535/1505/b505/8989/8997/5408/9341/5310/5510等IC已经实际测试	   	   
void LCD_Scan_Dir(u8 dir)
{
  	...
  	}else 
	{
		...
		if(lcddev.id==0x8989||lcddev.id==0X8997)//8989 IC
		{
			dirreg=0X11;
			regval|=0X6040;	//65K   
	 	}else//其他驱动IC		  
...
void LCD_DrawPoint(u16 x,u16 y)
{
	...
	{
		/*
		LCD_color[x][y] = POINT_COLOR;
		LCD_cord[fresh_index].x = x;
		LCD_cord[fresh_index].y = y;
		fresh_index ++;
		*/
	}
}
...
void LCD_Display_Dir(u8 dir)
{
	if(dir==0)			//竖屏
	{
		...
		}else if(lcddev.id==0X8989||lcddev.id==0X8997)
		{
			lcddev.wramcmd=R34;
	 		lcddev.setxcmd=0X4E;
			lcddev.setycmd=0X4F;  
...
{ 										  
  ...
  RCC_AHBPeriphClockCmd(RCC_AHBPeriph_FSMC,ENABLE);	//使能FSMC时钟
	...
	}else if(lcddev.id==0x8989||lcddev.id==0X8997)//OK |/|/|
	{	   
...
void LCD_CLearInterface(u16 TitleCmd,u16 Color)
{
	if(TitleCmd)
	{
		if(lcddev.dir==1)
			LCD_Fill(0,21,320,240,Color);
		else
			LCD_Fill(0,21,240,320,Color);
	}
	else
	{
		LCD_Clear(Color);
...
//(sx,sy)填充矩形左上角角坐标
//color:要填充的颜色
void LCD_FillSquare(u16 sx,u16 sy,u16 size,u16 color)
{
	LCD_Fill(sx,sy,sx+size,sy+size,color);
}
...
//r    :半径
// mode :0为空心，1为实心
void Draw_Circle(u16 x0,u16 y0,u8 r,u8 mode)
{
	int a,b,c;
	int di;
	a=0;b=r,c=0;	  
...
{         
    ...
    {       
		...
		if(!mode)
			LCD_Fill(x,y,width,height,BACK_COLOR);
}
...
//size:字体大小
//*p:字符串起始地址	
void LCD_ShowSymStr(u16 x,u16 y,u16 width,u16 height,u8 size,u8 *p,u8 mode)
{
	...
	{
		if(size == 12)
			strPixelNum = 6 * strlen((const char*)p);
		else
			strPixelNum = 8 * strlen((const char*)p);
	}
	else
		strPixelNum = width ;
	retractPixelNum =strPixelNum/2;
	
	LCD_ShowString(x - retractPixelNum,y - size/2,width,height,size,p,mode);
}
//从矩阵刷新ＬＣＤ（由于ＳＡＲＭ无法开大数组所以暂时无法实现）
//需要将.h中的 FRESH_MODE 置为1，并开启坐标、颜色数组、矩阵
/*
void LCD_FreshFromMatrix()
{
	while(fresh_index >= 0)
	{
		LCD_Fast_DrawPoint(LCD_cord[fresh_index].x,LCD_cord[fresh_index].y,LCD_color[LCD_cord[fresh_index].x][LCD_cord[fresh_index].y]);
		if(fresh_index>0)
			fresh_index--;
		else
			break;
...
*/
  ...
  //32位的RGB转换为RGB565
//RGB565和RGB555都是RGB格式，RGB24和RGB32则位BGR和BGRA格式
uint16_t RGB_to_RGB565(uint32_t RGB32_color)
{
	uint8_t blue = ((RGB32_color&0xff000000)>>24);
	uint8_t green = ((RGB32_color&0x00ff0000)>>16);
	uint8_t red = ((RGB32_color&0x0000ff00)>>8);
	
	return RGB565_Calculate(red,green,blue);
}
//由8位的调色板合成RGB565
uint16_t RGB565_Calculate(uint8_t red,uint8_t green ,uint8_t blue)
{
	uint16_t RGB565_temp = 0;
	RGB565_temp |=(red>>3)<<11;
	RGB565_temp |=(green>>2)<<5;
	RGB565_temp |=(blue>>3);
	return RGB565_temp;
}
